...
class Validator {
  ...
  #RULE_FUNCTIONS = {
    ...
    email: (value) => (value !== undefined && typeof value === 'string' && !this.#EMAIL_REGEX.test(value)),
    password: (value) => (
      value !== undefined
      && typeof value === 'string'
      && !(
  ...
  #validateByArray(rule, field) {
    ...
    for (let i = 0, len = rule.length; i < len; i += 1) {
      ...
      const isNotValid = this.#RULE_FUNCTIONS[name](field, value);

      if (isNotValid) {
        const message = this.#RULE_MESSAGES[name](value, typeof field === 'string');
